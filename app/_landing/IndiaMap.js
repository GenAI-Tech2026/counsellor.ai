...
/* Accurate, clearly-recognizable map of India used as a warm decoration to
   show counsa works across states. Pins glow at exam-heavy states; a few
   carry floating HTML labels.

   Outline source: mapsicon (https://github.com/djaiss/mapsicon), file
   all/in/vector.svg — a potrace-traced India country silhouette. mapsicon is
   released for free use ("Available for free. Do whatever you want with it.").
   The artwork keeps the source's own coordinate system: the path lives in a
   ~0..10240 space that is mapped into the 0..1024 viewBox by the potrace
   transform `translate(0,1024) scale(0.1,-0.1)`. We reuse that transform
   verbatim (on both the rendered land and the clip path) so the geometry is
   never distorted, and set the SVG viewBox to 0 0 1024 1024 with
   preserveAspectRatio="xMidYMid meet". A vector-effect="non-scaling-stroke"
   keeps the accent stroke a sensible width despite the 0.1x group scale. */

// potrace coordinate system -> 1024x1024 viewBox
const TRANSFORM = 'translate(0,1024) scale(0.1,-0.1)';

// Single combined national outline (clockwise from Kashmir), mapsicon all/in.
...
// pins: [x, y, hot?] in the 0..1024 viewBox (post-transform) coordinate space.
// hot = exam-heavy southern states, shown in green. Coordinates were derived
// by mapping each city's lat/long onto the rendered map and verified visually.
const PINS = [
  [330, 300],         // Delhi (north-center)
  [690, 500],         // West Bengal / Kolkata (east coast)
  [187, 637],         // Maharashtra / Mumbai (west coast)
  [372, 696, true],   // Telangana / Hyderabad (center-south)
  [441, 728, true],   // Andhra Pradesh (south-east of Telangana)
  [342, 852, true],   // Karnataka / Bengaluru (south-center-west)
  [430, 848, true],   // Tamil Nadu / Chennai (south-east coast)
  [300, 959, true],   // Kerala (south-west tip)
];

// labels: [x, y, text] in viewBox coords (0..1024); .tag floats just above
const LABELS = [
  [330, 300, 'Delhi'],
  [372, 696, 'Telangana'],
  [187, 637, 'Maharashtra'],
...
export default function IndiaMap() {
  return (
    <div className={m.wrap}>
      <svg className={m.map} viewBox="0 0 1024 1024" preserveAspectRatio="xMidYMid meet" role="img" aria-label="counsa.ai works across India">
        <defs>
          <linearGradient id="indiaFill" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#ffe3cf" />
            <stop offset="100%" stopColor="#ffc09c" />
          </linearGradient>
          <pattern id="indiaDots" width="34" height="34" patternUnits="userSpaceOnUse">
            <circle cx="5" cy="5" r="3" className={m.dots} />
          </pattern>
          <clipPath id="indiaClip"><path transform={TRANSFORM} d={INDIA} /></clipPath>
        </defs>

        <path className={m.land} vectorEffect="non-scaling-stroke" transform={TRANSFORM} d={INDIA} />
        <rect x="0" y="0" width="1024" height="1024" fill="url(#indiaDots)" clipPath="url(#indiaClip)" />

        {PINS.map(([x, y, hot], i) => (
          <g key={i}>
            <circle className={m.ring} cx={x} cy={y} r="18" style={{ animationDelay: `${i * 0.32}s` }} />
            <circle className={`${m.dot} ${hot ? m.dotHot : ''}`} cx={x} cy={y} r="12" />
          </g>
        ))}
      </svg>

      {LABELS.map(([x, y, text]) => (
        <span key={text} className={m.tag} style={{ left: `${(x / 1024) * 100}%`, top: `${(y / 1024) * 100}%` }}>{text}</span>
      ))}
    </div>
  );
}
