...
// Enabled whenever a REST URL + token are configured. In local dev these point at
// the auto-started local cache (scripts/dev.mjs → scripts/local-cache-server.mjs);
// in production set them to your Upstash database. When unset, every caller falls
// back to its non-Redis path (Postgres rate-limit, in-memory cache) — and even
// when set, any call that errors/times out degrades gracefully (post() returns null).
