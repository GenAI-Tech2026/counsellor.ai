...
async function getExtractor() {
  if (!_extractorPromise) {
    _extractorPromise = (async () => {
      ...
      // LOAD THE MODEL FROM DISK, NEVER THE NETWORK. The weights (~34MB, the
      // quantized ONNX + tokenizer) are committed under `models/` and bundled
      // into the Vercel function via `outputFileTracingIncludes` (see
      // next.config.mjs). Fetching them from the HuggingFace CDN at runtime is
      // exactly the kind of external dependency that has broken this path before
      // (the retired inference API → empty retrievals) and, on a cold serverless
      // start, a 34MB download can also blow past the request budget. Bundling
      // makes embedding fully self-contained: no API key, no network, no
      // cold-download stall — the model is just there on the function's disk.
      env.allowLocalModels = true;
      env.allowRemoteModels = false;                       // never hit the network
      env.localModelPath = path.join(process.cwd(), 'models'); // → models/Xenova/bge-small-en-v1.5
