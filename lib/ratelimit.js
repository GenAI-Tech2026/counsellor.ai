...
const MAX_PER_HOUR = 30;       // signed-in users
// Guest (signed-out) hourly cap. Configure via GUEST_CHAT_LIMIT_PER_HOUR.
// Defaults to 5 when the env var is unset, non-numeric, or <= 0 (see below).
// The client already forces sign-in after a few guest messages, so this cap is
// purely an abuse backstop for direct API hits — keeps a scripted guest from
// running up the Gemini bill. The global daily circuit-breaker
// (CHAT_GLOBAL_DAILY_MAX) is an additional backstop. Set
// GUEST_CHAT_LIMIT_PER_HOUR to a higher number to loosen the cap.
