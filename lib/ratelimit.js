...
// Guest (signed-out) hourly cap. 0 = disabled (no guest limit). Configure via
// GUEST_CHAT_LIMIT_PER_HOUR; currently disabled by default. NOTE: with this off,
// the only abuse backstop for guests is the global daily circuit-breaker
// (CHAT_GLOBAL_DAILY_MAX) — set this back to e.g. 5 to re-enable per-guest caps.
const GUEST_MAX_PER_HOUR = parseInt(process.env.GUEST_CHAT_LIMIT_PER_HOUR ?? '0', 10);
