...
//           GUEST_MAX_PER_HOUR (50 by default) for signed-out visitors.
...
// Defaults to 20 when the env var is unset, non-numeric, or <= 0 (see below).
// The client already forces sign-in after a few guest messages, so this cap is
// purely an abuse backstop for direct API hits — 20/hr is ample for a real
// guest and keeps a scripted guest from running up the Gemini bill. The global
// daily circuit-breaker (CHAT_GLOBAL_DAILY_MAX) is an additional backstop. Set
// GUEST_CHAT_LIMIT_PER_HOUR higher to loosen the cap.
...
if (isNaN(GUEST_MAX_PER_HOUR) || GUEST_MAX_PER_HOUR <= 0) {
  GUEST_MAX_PER_HOUR = 20;
}

// Global daily circuit-breaker. Caps total chat requests/day so abuse that slips
// past per-key limits (e.g. a botnet, or spoofed guest IPs) can't run up an
// unbounded LLM bill. Redis-only by design (a global counter shouldn't add a
// write to the primary DB on every request) and it fails OPEN, so it only ever
// trips on a real, measured spike — it can't take down chat on its own.
//
// Defaults to 10000/day so there is a safety ceiling out of the box; RAISE it via
// CHAT_GLOBAL_DAILY_MAX once you know your real daily volume, or set it to 0 to
// explicitly disable the breaker.
const _globalMaxEnv = process.env.CHAT_GLOBAL_DAILY_MAX;
const GLOBAL_DAILY_MAX = _globalMaxEnv != null && _globalMaxEnv !== ''
  ? parseInt(_globalMaxEnv, 10)
  : 10000;
