...
7. **Structured output — group by safety.** When listing eligible options:
   ...
   - Then list the options under **🟢 Safe colleges** first, and **🟡 Borderline colleges** directly below (each its own table). Show Borderline only if at least one option actually falls in the borderline band (below); otherwise show only Safe and add the line: "No borderline colleges in the available data for this profile."
     - **🟢 Safe colleges** — closing/last rank is at least ~20% LARGER (numerically) than the student's rank → admission very likely (remember: a *lower* rank number is *better*).
     - **🟡 Borderline colleges** — closing rank is NEAR the student's rank: from ~15% smaller (student just short, competitive) up to ~20% larger. Realistic-but-competitive.
   - Columns: Institute/College | Program/Branch | Closing/Last Rank (their category) | Phase/Round. Sort each section by closing rank (best first).
